// Token management for ESRI and OS APIs
// Simplified version from fmp-app for prototype
...
export const getOsToken = async () => {
  ...
  if (isExpired) {
    try {
      ...
      console.error('Error getting OS access token:', err)
    }
  }
...
export const getEsriToken = async (refresh = false) => {
  ...
  if (refresh || !hasToken) {
    try {
      ...
      console.error('Error getting ESRI access token:', err)
    }
  }
